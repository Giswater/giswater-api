...
class HydrantReachabilityUpdate(BaseModel):
    hydrant_id: str = Field(..., title="Hydrant ID", description="ID of the hydrant to update", examples=["J1"])
    value: float = Field(..., title="Value", description="Value", examples=[31.33, 2.26])

class ReservoirValueUpdate(BaseModel):
    reservoir_id: str = Field(..., title="Reservoir ID", description="ID of the reservoir to update", examples=["R1"])
...
class ValveValueUpdate(BaseModel):
    valve_id: str = Field(..., title="Valve ID", description="ID of the valve to update", examples=["V1"])
...
class TankValueUpdate(BaseModel):
    tank_id: str = Field(..., title="Tank ID", description="ID of the tank to update", examples=["T1"])
...
class JunctionValueUpdate(BaseModel):
    junction_id: str = Field(..., title="Junction ID", description="ID of the overflow structure to update", examples=["O1"])
    parameter: str = Field(..., title="Parameter", description="Parameter to update", examples=["height", "width"])
    value: float = Field(..., title="Value", description="Value", examples=[2.0, 1.5])

class PatternValueUpdate(BaseModel):
    pattern_id: str = Field(..., title="Pattern ID", description="ID of the pattern to update", examples=["P1"])
...
class ControlValueUpdate(BaseModel):
    ...
    parameter: str = Field(..., title="Parameter", description="Parameter to update", examples=["setpoint", "threshold"])
    value: Any = Field(..., title="Value", description="Value", examples=[5.0, 10.0])
